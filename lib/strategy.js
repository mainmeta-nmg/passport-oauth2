  ...
  if (req.query && req.query.code) {
    function loaded(err, ok, state) {
      ...
      self._oauth2.getOAuthAccessToken(code, params,
        function(err, _accessToken, _refreshToken, params) {
          if (err) { return self.error(self._createOAuthError('Failed to obtain access token', err)); }

          const accessToken = _accessToken || params.data.access_token;
          const refreshToken = _refreshToken || params.data.refresh_token;
          if (!params.expires_in) params.expires_in = params.data.expires_in;
